...
public class PayUtils {
    private static Logger logger = Logger.getLogger(String.valueOf(PayUtils.class));

    /**请求支付模块**/
    public static JSONObject wxPay(String openid,String orderNo,String title,String money, HttpServletRequest request){
        ...
        try{
            ...
            packageParams.put("total_fee", money);//订单金额
    ...
    /**查询订单**/
    public static JSONObject verifyOrderStatus(String orderNo){
        JSONObject json = new JSONObject();
        try{
            //生成的随机字符串
            String nonce_str = Utils.getRandomStringByLength(32);
            ...
            //调用统一下单接口，并接受返回的结果
            String result = PayUtils.httpRequest(WXConst.verify_url, "POST", xml);
            ...
            if(return_code == "SUCCESS" || return_code.equals(return_code)){
                // 业务结果
                trade_state = (String) map.get("trade_state");//返回交易状态
            }
            json.put("errMsg", "OK");
            json.put("trade_state", trade_state);
        }catch(Exception e){
            e.printStackTrace();
            json.put("errMsg", "Failed");
        }
